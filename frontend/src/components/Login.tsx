...
export default function Login({ onLoginSuccess }: LoginProps) {
  ...
  return (
    <div className="w-full max-w-md p-8 rounded-3xl glassmorphism transition-all duration-300 relative overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="mb-8 text-center">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl mb-4 transition-transform hover:scale-105 duration-300 bg-white/5 border border-white/10">
            {brand?.logo_url ? (
          ...
          <p className="text-xs mt-1.5 text-[#7A8B84]">
            {brand ? 'Inicia sesión para ingresar a la academia' : 'Inicia sesión para gestionar tus academias'}
          </p>
        </div>

        {error && (
          <div className="mb-6 p-4 rounded-xl text-sm border flex items-start gap-3 bg-red-500/5 border-red-500/20 text-[#FF5A5A]">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-[10px] font-bold uppercase tracking-wider mb-2 text-[#7A8B84]">
              Correo Electrónico
            </label>
            <input
              type="email"
              ...
              disabled={loading}
              className="w-full px-4 py-3 rounded-xl border border-[#1E2724] outline-none transition-all duration-200 text-sm bg-[#060908]/60 text-[#E6ECE9] input-glow"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 px-4 rounded-xl font-bold text-sm transition-all duration-300 flex items-center justify-center gap-2 transform active:scale-[0.98] btn-primary cursor-pointer disabled:cursor-not-allowed"
          >
            {loading ? (
              <>
